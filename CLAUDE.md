...
Look Up's head also carries the **seed clause** (`oracleSeedFromHTML`), the same `Cast from Sep 12 · 39-10-26-30-09-29` the card at the top prints. Without it the two surfaces show identical numbers with no visible reason they agree. Both rows therefore carry `oseed-row`, so the seed clause sits inline after the name and the confirmed jackpot takes its own full-width row beneath.
