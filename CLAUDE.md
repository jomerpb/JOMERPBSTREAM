...
**There are TWO reader cards — MangaFreak and Comix — and one mechanism.**
...
takes a reader key, so a third reader is a new entry plus its markup and nothing
else. Do not fork these into another copy. Every change here has been verified
to leave the other card behaving identically — same src, same height, same
request count, same open state, measured against the previous `main`.

A **WebComics** card lived here too and was removed at the repo owner's request,
together with its slug index, scraper, workflow and test. It is all in the
history if it is ever wanted back. The measurement that shaped it is worth
keeping in mind before adding any similar source: WebComics is a licensed
platform carrying its own originals, and only **41 of 967 hold-out AniList
titles (4.2%)** resolved to a page on it — confirmed as a property of the
catalogue rather than the matcher by asking its own search for 18 titles the
matcher missed, with 5 known-present controls (control 5/5, and 0 of the 18
genuinely present).
...
be. The repo owner was asked and chose search-only, so there is no index, no
scraper and no workflow for this source. **Do not add one without asking them
again.**
...
**MangaFreak starts open; Comix starts collapsed.** Comix is a second 78vh frame
on one page, and a collapsed card costs **zero requests** — measured, not
inferred from an absent `src` — which matters more here than anywhere given the
unverified rendering above.
