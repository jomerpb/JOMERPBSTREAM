...
// ══════════════════════════════════════════════════════════════
// ORACLE REPORT CARD — triggers oracle-backtest.yml (same token +
// dispatch flow as the PCSO "Fetch Live" button and the Trade tab's
// Report Card, via the shared tpGetGithubToken()/pcsoDispatch() already
// used above by pcsoAIFetch), which replays computeOracleAsOf() over
// every historical PCSO draw in this repo's own pcso-history.json and
// grades it against two random baselines: the exact hypergeometric
// theoretical mean, and a live Monte Carlo simulation. Results land in
// oracle-backtest.json (backtest_oracle.mjs), rendered below in plain
// language. Read-only against the data pipeline — see that script's
// header comment.
// ══════════════════════════════════════════════════════════════
var ORACLE_BT_WORKFLOW='oracle-backtest.yml';
var oracleBacktestReloadTimer=null;
var oracleBacktestCollapsed=false;

function oracleBulletsHTML(parts){
  var items=(Array.isArray(parts)?parts:[parts]).filter(function(p){return p&&String(p).trim();});
  return '<ul class="tp-summary-bullets">'+items.map(function(p){return '<li>'+p+'</li>';}).join('')+'</ul>';
}

function oracleToggleBacktestBody(){
  oracleBacktestCollapsed=!oracleBacktestCollapsed;
  var txt=document.getElementById('oracle-backtest-text');
  var tag=document.getElementById('oracle-backtest-tag');
  if(txt) txt.style.display=oracleBacktestCollapsed?'none':'block';
  if(tag) tag.textContent='ORACLE REPORT CARD '+(oracleBacktestCollapsed?'▸':'▾');
}

async function oracleTriggerBacktest(){
  var btn=document.getElementById('oracle-backtest-btn');
  var status=document.getElementById('oracle-backtest-status');
  var token=tpGetGithubToken();
  if(!token){ if(status) status.textContent='Cancelled — no token entered.'; return; }

  if(btn) btn.disabled=true;
  if(status) status.textContent='Triggering Oracle backtest + simulation...';
  ...
  try{
    await pcsoDispatch(ORACLE_BT_WORKFLOW, ghHeaders);
    if(status) status.textContent='Backtest running ✅ — report reloads here in ~90s.';
    if(oracleBacktestReloadTimer) clearTimeout(oracleBacktestReloadTimer);
    oracleBacktestReloadTimer=setTimeout(function(){
      oracleLoadBacktest();
      if(status) status.textContent='Report card refreshed.';
      if(btn) btn.disabled=false;
    }, 90*1000);
  }catch(e){
    if(status) status.textContent='Error ❌ — '+(e&&e.message?e.message:'see console');
    if(btn) btn.disabled=false;
  }
}

var ORACLE_BT_GAME_LABELS={'642':'6/42','645':'6/45','649':'6/49','655':'6/55','658':'6/58'};

function oracleBtFmt(x){ return (x==null)?'—':x.toFixed(3); }

// Renders oracle-backtest.json as plain language: what was tested, how the
// Oracle's picks actually did against real historical draws compared to
// pure chance, and — because chance is the mathematically correct
// expectation for an independent random lottery draw — an honest read on
// whether any deviation from chance is real or just sampling noise.
function oracleRenderBacktest(data){
  var el=document.getElementById('oracle-backtest-text');
  if(!el) return;
  var parts=[];

  var sixBall=data.sixBall||{};
  var gameKeys=Object.keys(ORACLE_BT_GAME_LABELS).filter(function(k){return sixBall[k];});
  var totalN=gameKeys.reduce(function(a,k){return a+(sixBall[k].n||0);},0);
  var dateFrom=null,dateTo=null;
  gameKeys.forEach(function(k){
    var dr=sixBall[k].dateRange||{};
    if(dr.from&&(!dateFrom||dr.from<dateFrom)) dateFrom=dr.from;
    if(dr.to&&(!dateTo||dr.to>dateTo)) dateTo=dr.to;
  });

  parts.push("What this is: for every past 6-ball draw in this site's own stored PCSO history, the Oracle's exact pick was recomputed using ONLY draws strictly before that date (the same freeze the live tab uses) via <code>computeOracleAsOf()</code> — then graded against what actually came up, and against two random baselines: the exact hypergeometric math for a pure-chance guess, and a live Monte Carlo simulation of "+((data.simsPerDraw||0).toLocaleString())+" random picks per draw. Graded <b>"+totalN.toLocaleString()+" draws</b> across the 5 6-ball games"+(dateFrom?" ("+dateFrom+" to "+dateTo+")":"")+(data.generatedAt?", last run "+data.generatedAt.slice(0,10):"")+".");

  gameKeys.forEach(function(k){
    var s=sixBall[k];
    var label=ORACLE_BT_GAME_LABELS[k];
    var verdict=s.edgeWithinNoise===false
      ? "outside the noise band — a real, measured deviation from chance"
      : "within normal sampling noise — statistically indistinguishable from a random guess";
    parts.push("<b>"+label+"</b> ("+s.n+" draws): Oracle averaged <b>"+oracleBtFmt(s.oracle.avgMatches)+"</b> matches/draw vs <b>"+oracleBtFmt(s.theoretical.avgMatches)+"</b> expected by pure chance (Monte Carlo sim landed at "+oracleBtFmt(s.monteCarlo.avgMatches)+", confirming the simulator itself is accurate). Edge: "+(s.edge>=0?'+':'')+oracleBtFmt(s.edge)+" ± "+oracleBtFmt(s.edgeStdError)+" — "+verdict+".");
  });

  var ez2=data.ez2&&data.ez2.hours;
  if(ez2){
    var ez2Bits=['2PM','5PM','9PM'].filter(function(h){return ez2[h];}).map(function(h){
      var s=ez2[h];
      return h+": "+oracleBtFmt(s.oracle.avgMatches)+" vs "+oracleBtFmt(s.theoretical.avgMatches)+" chance ("+(s.edgeWithinNoise===false?'outside noise':'within noise')+")";
    });
    if(ez2Bits.length) parts.push("<b>EZ2</b> (2-number, drawn independently so doubles like 22-22 happen): "+ez2Bits.join(' · ')+".");
  }

  var allWithinNoise=gameKeys.every(function(k){return sixBall[k].edgeWithinNoise!==false;})
    && (!ez2 || ['2PM','5PM','9PM'].every(function(h){return !ez2[h]||ez2[h].edgeWithinNoise!==false;}));
  parts.push("<b>Honest fine print:</b> "+(allWithinNoise
    ? "every game above currently sits within sampling noise of pure chance — which is the mathematically correct, expected outcome for genuinely random lottery draws, not a flaw in the Oracle. No numerology/astrology/BaZi/feng-shui/I-Ching/tarot layer, and no statistics layer, can shift the real odds of an independent random draw."
    : "at least one game above shows a deviation outside the expected noise band. With this many games and hours tested, a small number of such 'hits' can occur by chance alone (multiple-comparisons effect) — treat it as something to keep watching over more draws, not as proof of an edge.")
    +" This report exists to keep that claim checkable against this site's own draw history, and to catch a broken simulator (see the Monte-Carlo-vs-theoretical agreement above) rather than to make the Oracle look more predictive than a real lottery can be.");

  el.innerHTML=oracleBulletsHTML(parts);
}

async function oracleLoadBacktest(){
  var el=document.getElementById('oracle-backtest-text');
  if(!el) return;
  var RAW_URL='oracle-backtest.json'; // same-origin via GitHub Pages
  try{
    var resp=await pcsoFetchWithTimeout(RAW_URL+'?nocache='+Date.now(),8000);
    if(!resp.ok) throw new Error('HTTP '+resp.status);
    var data=await resp.json();
    oracleRenderBacktest(data);
  }catch(e){
    el.innerHTML=oracleBulletsHTML([
      "No report card yet. Tap the <b>Report Card</b> button above to run the first one — it rewinds through this site's own stored PCSO history, replays what the Oracle would have picked on every past draw date (using only data it would have had that day), and grades those picks against two random baselines: the exact math for pure chance, and a live Monte Carlo simulation. Takes about 90 seconds.",
      "Why it matters: lottery draws are independent random events — no system, including this one, can shift the real odds. This report exists to keep that claim honest and checkable against this site's own draw history, rather than asking you to trust it on faith."
    ]);
  }
}
oracleLoadBacktest();
