...
// ══════════════════════════════════════════════════════════════
// SCALPING FILTER — detection logic reset 2026-07-14, rebuilding from
// scratch. Everything below the constants (shortlist, intraday fetch,
// cache, auto-rescan timer, manual re-tap-to-refresh, tab/UI rendering)
// is the working data-refresh shell and stays as-is. tcAnalyzeScalpSetup
// is now a stub that returns null for every coin — no pattern-matching,
// no thresholds, no "qualifies" logic — until new detection rules are
// written in from scratch.
// ══════════════════════════════════════════════════════════════
var TC_SCALP_SHORTLIST_SIZE = 18;        // only daily-non-bearish coins get an intraday fetch
var TC_SCALP_CACHE_MS = 4*60*1000;       // refresh cadence (~4 min) — auto-rescan + manual re-tap
...
// STUB — old pattern-detection logic removed 2026-07-14, rebuilding from
// scratch. Takes the same (bars) input the refresh shell already fetches
// per coin, returns null for now (no coin "qualifies" until new rules are
// written here). Keep the same return shape in mind for downstream code
// (tcRunScalpScan pushes {sym,name,price,pct,scalp}; tcRenderTop and
// tcRenderAll's SCALP RANGE CHECK read fields off r.scalp) once real
// logic replaces this stub.
function tcAnalyzeScalpSetup(bars){
  return null;
}

// Refresh shell kept as-is: Stage 1 shortlists candidates from data already
// loaded (free, instant); Stage 2 spends real intraday API calls ONLY on
// that shortlist, paced to stay well inside the keyless tier's limits.
// Stage 3 (tcAnalyzeScalpSetup) is currently a stub returning null for
// every coin, so `results` will always come back empty until new detection
// logic is written — the fetch/cache/pacing pipeline itself still runs
// each cycle so the refresh mechanism stays provably working.
async function tcRunScalpScan(){
  ...
  try {
    ...
    for(var i=0;i<shortlist.length;i++){
      ...
      if(bars){
        var setup = tcAnalyzeScalpSetup(bars); // stub — always null for now
...
function tcRenderTop(){
  ...
  if(tcGainersModeVal === 'bullish'){
    ...
    if(tcScalpScanState.loading){
      el.innerHTML = '<div class="tp-gainer-empty">Refreshing intraday data for '+TC_SCALP_SHORTLIST_SIZE+' coins \u2014 pulls live 5-min data per coin, ~'+Math.round(TC_SCALP_SHORTLIST_SIZE*TC_SCALP_CALL_SPACING_MS/1000)+'s.</div>';
      if(noteEl) noteEl.textContent = '';
      return;
    }
  ...
  if(!rows.length){
    el.innerHTML = tcGainersModeVal === 'scalping'
      ? '<div class="tp-gainer-empty">Scalping detection logic is being rebuilt from scratch \u2014 no criteria are applied yet, so no coins will show here. Data still refreshes every '+(TC_SCALP_CACHE_MS/60000)+' min.</div>'
  ...
  if(noteEl){
    noteEl.textContent = tcGainersModeVal === 'bullish'
      ...
      ? 'Detection logic reset \u2014 rebuilding from scratch. The refresh pipeline (real 5-min CoinGecko data, not daily bars) is still live and rescans every '+(TC_SCALP_CACHE_MS/60000)+' min; new setup criteria will show here once written.'
...
// ── Funds / buying Power (localStorage 'tcFunds') — global block, lives in
// index.html OUTSIDE the rebuilt list so removing coins can never touch it.
// Power moves ONLY on Deposit / Withdraw / a confirmed Save (user 2026-07-11):
// buy deducts total incl. tax, sell adds net proceeds (realized G/L is baked
// into the proceeds by construction — no separate G/L posting needed). ──
var TC_FUNDS_KEY = 'tcFunds';
function tcGetFunds(){
  try { var f = JSON.parse(localStorage.getItem(TC_FUNDS_KEY)||'{}'); return {power: isFinite(f.power) ? +f.power : 0}; }
  catch(e){ return {power:0}; }
}
function tcSaveFunds(f){
  try { localStorage.setItem(TC_FUNDS_KEY, JSON.stringify(f)); } catch(e){}
}
// Cash balances are always 2dp — tcFmtPHP's 4dp sub-\u20b11 branch is for coin
// prices, not money, so \u20b10 must read \u20b10.00 rather than \u20b10.0000.
function tcFmtPHPCash(n){
  if(n == null || !isFinite(n)) return '\u2014';
  return '\u20b1' + n.toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});
}
// Numbers-only guard for Pocket money: digits + a single decimal point.
function tcFundsFilterInput(el){
  var v = el.value.replace(/[^0-9.]/g,'');
  var i = v.indexOf('.');
  if(i >= 0) v = v.slice(0, i+1) + v.slice(i+1).replace(/\./g,'');
  el.value = v;
}
function tcFundsMsg(txt){
  var m = document.getElementById('tc-funds-msg');
  if(m) m.textContent = txt || '';
}
function tcFundsAmount(){
  var el = document.getElementById('tc-funds-pocket');
  var n = el ? parseFloat(el.value) : NaN;
  return (!isNaN(n) && n > 0) ? n : null;
}
function tcFundsDeposit(){
  var amt = tcFundsAmount();
  if(amt == null){ tcFundsMsg('Enter an amount above zero to deposit.'); return; }
  var f = tcGetFunds();
  f.power = +(f.power + amt).toFixed(2);
  tcSaveFunds(f);
  var el = document.getElementById('tc-funds-pocket'); if(el) el.value = '';
  tcFundsMsg('');
  tcRenderFunds();
}
function tcFundsWithdraw(){
  var amt = tcFundsAmount();
  if(amt == null){ tcFundsMsg('Enter an amount above zero to withdraw.'); return; }
  var f = tcGetFunds();
  if(amt > f.power + 1e-9){ tcFundsMsg('Not enough Power — you have '+tcFmtPHPCash(f.power)+'.'); return; }
  f.power = +(f.power - amt).toFixed(2);
  tcSaveFunds(f);
  var el = document.getElementById('tc-funds-pocket'); if(el) el.value = '';
  tcFundsMsg('');
...
// Unrealized G/L across OPEN positions (status 'bought') at live prices vs
// the confirmed buy cost; % against total cost basis. Sold rows are excluded
// — their realized G/L already reached Power inside the sell proceeds. Rows
// with no live quote are left out of both sums rather than guessed at.
function tcOpenPositionsGl(){
  var list = tcGetWatch(), gl = 0, basis = 0, n = 0;
  list.forEach(function(w){
    if(w.status !== 'bought' || !(w.buyTotal > 0)) return;
    var q = tcGetQuote(w.sym);
    if(!q || q.price == null) return;
    var fN = tcTradeFees((w.buyShares||0) * q.price, 'sell');
    if(!fN) return;
    gl += fN.total - w.buyTotal; basis += w.buyTotal; n++;
  });
  return {gl:gl, basis:basis, count:n};
}
function tcRenderFunds(){
  var pEl = document.getElementById('tc-funds-power');
  if(pEl) pEl.textContent = tcFmtPHPCash(tcGetFunds().power);
  var iEl = document.getElementById('tc-funds-info');
  if(!iEl) return;
  var o = tcOpenPositionsGl();
  if(!o.count){ iEl.innerHTML = 'No open positions — confirmed buys will show their live gain / loss here.'; return; }
  var pct = o.basis > 0 ? o.gl / o.basis * 100 : 0;
  var cls = o.gl >= 0 ? 'gain' : 'loss';
  var word = o.gl >= 0 ? 'gained' : 'lost';
  var sign = o.gl >= 0 ? '+' : '\u2212';
  iEl.innerHTML = 'You have '+word+' <b class="'+cls+'">'+sign+tcFmtPHPCash(Math.abs(o.gl))+' ('+sign+Math.abs(pct).toFixed(2)+'%)</b> on '+o.count+' open position'+(o.count>1?'s':'')+'.';
}
...
// Status-aware card numbers. Drafts keep the 2026-07-11 side-based formula
// untouched. Once a buy is CONFIRMED, Total (buy side) and G/L pin to the
// snapshotted buyTotal — the exact amount Power was debited — so the row,
// the Funds info line, and Power can never disagree; the sell side keeps
// computing live from the price field so you can plan the exit. Sold rows
// are frozen at the booked proceeds and realized G/L.
function tcPortNumbers(w, side, sh, en, cur){
  if(w && w.status === 'sold'){
    return {total: w.sellProceeds!=null ? w.sellProceeds : null, feeTitle:'',
            gl: w.realizedGl!=null ? w.realizedGl : null,
            glPct: (w.realizedGl!=null && w.buyTotal>0) ? w.realizedGl/w.buyTotal*100 : null,
            sellNow:null, sellNowFees:null};
  }
  if(w && w.status === 'bought'){
    ...
    if(side === 'sell'){
      ...
      // sell-side Total covers the QTY typed in the shares field (partial
      // sells, user 2026-07-12) — with the whole position entered it equals
      // the full liquidation value (the old on-card "If sold now", removed
      // from display 2026-07-12; G/L still uses the same number internally).
      ...
      var fb = tcTradeFees((w.buyShares||0)*(w.buyPrice||0), 'buy');
      out.total = w.buyTotal!=null ? w.buyTotal : null;
      out.feeTitle = fb ? tcPortFeeTitle(fb) : '';
    }
    if(cur != null){
      var fN = tcTradeFees((w.buyShares||0)*cur, 'sell');
      if(fN){
        out.sellNow = fN.total; out.sellNowFees = fN.fees;
        out.gl = fN.total - (w.buyTotal||0);
        out.glPct = w.buyTotal>0 ? out.gl/w.buyTotal*100 : null;
      }
    }
    return out;
  }
  var r = tcPortCompute(side, sh, en, cur);
  return {total:r.total, feeTitle:tcPortFeeTitle(r), gl:r.gl, glPct:r.glPct,
          sellNow:r.sellNow, sellNowFees:r.sellNowFees};
}
// Buy-timestamp pretty-printer for the row DETAILS panel — device locale,
// e.g. "Jul 12, 2026, 1:38 PM". Rows bought before buyAt existed show a dash.
function tcFmtWhen(ts){
...
function tcPortToggleDetails(uid){
  var box = document.getElementById('tc-port-det-'+uid);
  var tag = document.getElementById('tc-port-dettag-'+uid);
  if(!box) return;
...
// for the qty typed in the shares field — the qty is REQUIRED; an empty
// field is an error, never a full liquidation (user 2026-07-12). Full
...
function tcPortSave(uid){
  ...
  if(side === 'buy'){
    ...
    // shares field (editable in Sell mode) and MUST be typed — an empty qty
    // is an ERROR, never "sell everything" (user 2026-07-12); the field
    // starts blank in Sell mode so a background rebuild can never re-arm a
    // full-position sale. A qty below the held amount is a PARTIAL sell
    // (user 2026-07-12): proceeds go to Power, the sold slice's prorated
    // cost basis leaves the row, and the row stays 'bought' with the rest.
    var held = w.buyShares || 0;
    if(!(sh > 0)){ tcPortMsg(uid, 'Enter how many to sell \u2014 you hold '+held+'.'); return; }
    var qty = sh;
    ...
    if(qty >= held - 1e-9){
      ...
      // (the qty field starts blank — type how many of the rest to sell).
      w.side = 'buy';
      tcSaveWatch(list);
      tcRenderWatchlist();
...
function tcPortRecalc(uid){
  ...
  // sells, user 2026-07-12) and snap back to the held amount on Buy. The
  // sell qty itself is never written here — it starts blank (the placeholder
  // shows what's held) and must be typed (user 2026-07-12).
  if(w.status === 'bought'){
    var lockSh = side !== 'sell';
    shEl.readOnly = lockSh;
    if(lockSh && w.buyShares != null) shEl.value = w.buyShares;
    shEl.placeholder = lockSh ? '0.00' : 'of '+(w.buyShares!=null?w.buyShares:0)+' held';
  }
...
function tcSetPortField(uid, field, val){
  ...
  if(field==='side'){
    w.side = val==='sell' ? 'sell' : 'buy';
    // Flipping side on a bought row clears the transient sell ticket — the
    // qty must be typed fresh every time; nothing prefills (user 2026-07-12).
    if(w.status === 'bought'){
      var flipEl = document.getElementById('tc-port-shares-'+uid);
      if(flipEl){ flipEl.value=''; flipEl.placeholder = w.side==='sell' ? ('of '+(w.buyShares!=null?w.buyShares:0)+' held') : '0.00'; }
    }
  }
...
// Remove blocked, Power debited; shares re-open BLANK in Sell mode as the
...
function tcRenderWatchlist(){
  ...
  listEl.innerHTML = list.map(function(w){
    ...
    // Sell-armed rows render the qty BLANK — the sell ticket must be typed
    // fresh; the placeholder shows what's held (user 2026-07-12).
    var sellArm = status==='bought' && side==='sell';
    var shVal = sellArm ? '' : (status!=='draft' ? (w.buyShares==null?'':w.buyShares) : (w.shares==null ? '' : w.shares));
    var shPh = sellArm ? ('of '+(w.buyShares!=null?w.buyShares:0)+' held') : '0.00';
    ...
    // the sell qty (starts blank, must be typed), enabling partial sells
    // (user 2026-07-12). Locked on bought+Buy and on sold rows, where the
    // label flips to "# Sold" so a closed row can't read as a holding
    // (user 2026-07-12).
    ...
    return '<div class="tp-watch-row tc-port-row">'+
      ...
      '<div class="tc-port-grid tc-port-calcline">'+
        '<div class="tp-watch-inbox"><span class="tp-watch-inlabel">'+(status==='sold'?'# Sold':'# Shares')+'</span>'+
          '<input type="number" step="any" min="0" inputmode="decimal" class="tp-watch-input" id="tc-port-shares-'+uid+'" placeholder="'+shPh+'" value="'+shVal+'"'+(shLock?' readonly':'')+' oninput="tcPortRecalc(\''+uid+'\')" onchange="tcSetPortField(\''+uid+'\', \'shares\', this.value)"></div>'+
...
function tcRenderAll(sym){
  ...
  // SCALP RANGE CHECK — dormant until the new scalp calc (rebuilt from
  // scratch) populates tcSelectedScalpSetup with real fields again.
  // tcSelectedScalpSetup will stay null for now since tcAnalyzeScalpSetup
  // is a stub, so this note simply won't appear.
  var scalpFeeNote = '';
  if(tcSelectedScalpSetup && tcSelectedScalpSetup.rangeWidthPct != null){
    var sw = tcSelectedScalpSetup;
    scalpFeeNote = '<br><br><b>SCALP RANGE CHECK:</b> the current bounce is \u20b1'+sw.rangeLow.toFixed(4)+'\u2013\u20b1'+sw.rangeHigh.toFixed(4)+' ('+sw.rangeWidthPct+'% wide).';
  }
